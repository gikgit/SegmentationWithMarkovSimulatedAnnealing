...
/*return 1 when probabilite is p*/
int flip(float p)
{
  if(randomize()<=p)
    return(1);
...
/*calcule the probability of each 2 classes for temperature t*/
...
/*choose one label between 2 classes recording to the probabilites*/
...
float prob(float gray, float mean, float var)
{
   return (1/sqrt(2*PI*var))*exp(-(powf(gray-mean,2)/(2*var)));

}

int U(int i,int j, int label, float** x)
{
	int l,k,count=0;
